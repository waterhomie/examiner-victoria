...
- 第一版使用低成本模式、小规格、最小副本 0、最大副本 1 或 2，控制公测成本。
- 第一次部署选择“部署完成后保持流量为 0”，手动访问默认域名完成只读验收后再开放。
...
- 使用“新建版本”方式部署。
- 初次部署完成后先保持 0% 流量或仅人工访问。
- 若新版本失败，将流量切回旧版本或删除新版本。
...
7. 配置环境变量，不复制到仓库。
8. 选择低成本模式与小规格。
9. 部署完成后先保持流量为 0 或仅人工验证。
10. 记录默认域名与版本号。
11. 验证 health、diagnostics、System check。
